...
class OnlineStatusConsumer(WebsocketConsumer):
    ...
    def online_status_handler(self, event):
        ...
        public_chat_users = ChatGroup.objects.get(group_name='gang-gang').users_online.exclude(id=self.user.id)


        my_chats = self.user.chats_groups.all()
        private_chats_with_users = [chat
                                    for chat in my_chats.filter(is_private=True)
                                    if chat.users_online.exclude(id=self.user.id)
                                    ]
        group_chats_with_users = [chat
                                    for chat in my_chats.filter(group_name__isnull=False)
                                    if chat.users_online.exclude(id=self.user.id)
                                    ]
